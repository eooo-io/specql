...
/**
 * @fileoverview Main CLI entry point for specql.
 * Sets up the command-line interface using Commander.js and routes to command handlers.
 * @module index
 */
...
async function main() {
  ...
  program
    .command('init')
    .description(
      'Initialize a new database setup from an OpenAPI specification'
    )
    ...
    .action(async (spec, options) => {
      try {
        ...
        console.error(
          boxes.error(`${styles.error('Error:')} ${(error as Error).message}`)
        );
        process.exit(1);
      }
    });
