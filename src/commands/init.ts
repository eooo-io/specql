...
import { boxes, formatList, formatPath, formatSection, getSpinnerStyle, styles } from '../utils/terminal';
...
export async function initializeProject(specPath: string, options: InitOptions): Promise<void> {
  ...
  try {
    ...
    spinner.succeed(`OpenAPI specification loaded from ${styles.code(specPath)}`);
    ...
    spinner.succeed(`Found ${styles.highlight(schemas.length.toString())} schemas to process`);
    ...
    const generatedFiles = [
      ...
      ...(choices.generateTypes ? [`${formatPath(choices.outputPath)}/types/`] : []),
      ...(choices.generateQueries ? [`${formatPath(choices.outputPath)}/queries/`] : [])
    ];

    console.log('\n' + boxes.success(
      formatSection('Success!', 
  ...
  const questions = [
    {
      ...
      choices: [
        { name: `${styles.bullet} PostgreSQL ${styles.subtle('(recommended)')}`, value: DatabaseType.PostgreSQL },
        { name: `${styles.bullet} MySQL`, value: DatabaseType.MySQL },
        { name: `${styles.bullet} SQLite ${styles.subtle('(lightweight)')}`, value: DatabaseType.SQLite },
        { name: `${styles.bullet} Microsoft SQL Server`, value: DatabaseType.MSSQL }
      ]
    },
    {
      type: 'list',
      ...
      choices: [
        { name: `${styles.bullet} From schema titles ${styles.subtle('(human-readable)')}`, value: NamingStrategy.SchemaTitle },
        { name: `${styles.bullet} From schema IDs ${styles.subtle('(consistent)')}`, value: NamingStrategy.SchemaId },
        { name: `${styles.bullet} Custom naming pattern ${styles.subtle('(advanced)')}`, value: NamingStrategy.Custom }
      ]
    },
    {
      type: 'list',
    ...
    },
    {
      ...
      validate: (input: string) => input.trim().length > 0 || styles.error('Please enter a valid path')
    }
  ];
